...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const myName = getItem('name');
    if (!!myName) setName(myName)

    const myEmail = getItem('email');
    if (!!myEmail) setEmail(myEmail)

    const myid = getItem('id');
    if (!myid) setId(uuidV4());

    socket.on('previousMsg', (messages: iPayload[])=> {
      let oldMessage: iMessage[] = []
      ...
      setMassage(oldMessage)
    })

    socket.on('msgFromServer', (message: iPayload)=> {
  ...
  return (
    <div  className="flex flex-col items-center justify-center w-screen min-h-screen bg-gray-100 text-gray-800 p-2">
      <div className="flex flex-col flex-grow w-full max-w-xl bg-white shadow-xl rounded-lg overflow-hidden">
            ...
            <div className="grid grid-rows-2">
              ...
              <div className="flex flex-col leading-tight">
                <div className="text-2xl mt-1 flex items-center">
                  <input className={`"text-gray-700 mr-2 rounded-2xl px-4 py-2" ${!name ? 'bg-gray-200': 'font-semibold'}`}
                    value={name}
              ...
              <div className="flex flex-col leading-tight">
                <div className="text-2xl mt-1 flex items-center">
                  <input className={`"text-gray-700 mr-2 rounded-2xl px-4 py-2" ${!email ? 'bg-gray-200': 'font-semibold'}`}
                    value={email}
                    type='email'
        ...
        <div className="flex flex-col flex-grow h-0 p-4 gap-y-2 overflow-auto">
          {messages.map((message: iMessage) => {
            return (
              <Message key={message.id} isMy={message.email === email} id={message.id} name={message.name} text={message.text} image={myImage} />
            )
          })}
        </div>        
        ...
        <div className="border-t-2 border-gray-200 px-4 pt-4 mb-4">
          <div className="relative flex">
            ...
            <input type="text" placeholder={disabled ? 'Digite seu nome e email no cabeçalho' :'Digite sua menssagem!'}
